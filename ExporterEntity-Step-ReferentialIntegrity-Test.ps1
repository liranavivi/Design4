# ExporterEntity Referential Integrity Test
# Tests ExporterEntity DELETE and UPDATE validation against StepEntity references
# Follows the same comprehensive testing pattern as ImporterEntity
...
Write-Host "EXPORTERENTITY REFERENTIAL INTEGRITY TEST" -ForegroundColor Cyan
...
$protocolResult = Test-Scenario "Create Test Protocol" {
    $protocolData = @{
        ...
        description = "Test protocol for ExporterEntity referential integrity"
    }
...
# Test 2: Create ExporterEntity without references
Write-Host ""
Write-Host "2. CREATE EXPORTERENTITY FOR TESTING" -ForegroundColor Cyan
$script:totalTests++
$exporterResult = Test-Scenario "Create ExporterEntity" {
    ...
    $exporterData = @{
        name = "Test Exporter"
        version = "1.0"
        description = "Test exporter for referential integrity"
        protocolId = $script:testProtocolId
        inputSchema = "{}"
    }
    
    $result = Invoke-ApiCall -Method "POST" -Endpoint "/api/exporters" -Body $exporterData
    if ($result.Success) {
        $script:testExporterId = $result.Data.id
        Write-Host "    Created exporter: $($script:testExporterId)" -ForegroundColor Gray
        return $true
    } else {
        Write-Host "    Failed to create exporter: $($result.Error)" -ForegroundColor Red
        return $false
    }
}
if ($exporterResult) { $script:passedTests++ }

# Test 3: Delete ExporterEntity without references (should succeed)
Write-Host ""
Write-Host "3. DELETE EXPORTERENTITY WITHOUT REFERENCES" -ForegroundColor Cyan
$script:totalTests++
$deleteWithoutRefsResult = Test-Scenario "Delete ExporterEntity (No References)" {
    if (-not $script:testExporterId) {
        Write-Host "    Skipping - no exporter available" -ForegroundColor Yellow
        return $false
    }
    
    $result = Invoke-ApiCall -Method "DELETE" -Endpoint "/api/exporters/$($script:testExporterId)"
    if ($result.Success) {
        Write-Host "    Exporter deleted successfully" -ForegroundColor Gray
        return $true
    } else {
        Write-Host "    Failed to delete exporter: $($result.Error)" -ForegroundColor Red
        return $false
    }
}
if ($deleteWithoutRefsResult) { $script:passedTests++ }

# Test 4: Create new ExporterEntity for reference testing
Write-Host ""
Write-Host "4. CREATE EXPORTERENTITY FOR REFERENCE TESTING" -ForegroundColor Cyan
$script:totalTests++
$exporterResult2 = Test-Scenario "Create ExporterEntity 2" {
    if (-not $script:testProtocolId) {
        Write-Host "    Skipping - no protocol available" -ForegroundColor Yellow
        return $false
    }
    ...
    $exporterData = @{
        ...
        version = "2.0"
        description = "Test exporter for referential integrity with references"
        protocolId = $script:testProtocolId
        inputSchema = "{}"
    }
...
if ($exporterResult2) { $script:passedTests++ }

# Test 5: Create StepEntity with ExporterEntity reference
Write-Host ""
Write-Host "5. CREATE STEPENTITY WITH EXPORTERENTITY REFERENCE" -ForegroundColor Cyan
$script:totalTests++
$stepResult = Test-Scenario "Create StepEntity" {
    ...
    $stepData = @{
        entityId = $script:testExporterId  # Reference our test exporter
        nextStepIds = @()
...
# Test 6: Delete ExporterEntity with references (should fail)
Write-Host ""
Write-Host "6. DELETE EXPORTERENTITY WITH REFERENCES (SHOULD FAIL)" -ForegroundColor Cyan
$script:totalTests++
$deleteWithRefsResult = Test-Scenario "Delete ExporterEntity (With References)" {
    if (-not $script:testExporterId) {
        Write-Host "    Skipping - no exporter available" -ForegroundColor Yellow
        return $false
    }
...
# Test 8: Update ExporterEntity with references (should fail)
Write-Host ""
Write-Host "8. UPDATE EXPORTERENTITY WITH REFERENCES (SHOULD FAIL)" -ForegroundColor Cyan
$script:totalTests++
$updateWithRefsResult = Test-Scenario "Update ExporterEntity (With References)" {
    if (-not $script:testExporterId) {
        Write-Host "    Skipping - no exporter available" -ForegroundColor Yellow
        return $false
    }
    ...
    $getResult = Invoke-ApiCall -Method "GET" -Endpoint "/api/exporters/$($script:testExporterId)"
    if (-not $getResult.Success) {
        Write-Host "    Failed to get exporter for update" -ForegroundColor Red
        return $false
    }

    $exporter = $getResult.Data
    $exporter.description = "Updated description - should fail due to references"

    $result = Invoke-ApiCall -Method "PUT" -Endpoint "/api/exporters/$($script:testExporterId)" -Body $exporter
    if (-not $result.Success -and $result.StatusCode -eq 409) {
...
# Test 10: Delete ExporterEntity after removing references (should succeed)
Write-Host ""
Write-Host "10. DELETE EXPORTERENTITY AFTER REMOVING REFERENCES" -ForegroundColor Cyan
$script:totalTests++
$deleteAfterCleanupResult = Test-Scenario "Delete ExporterEntity (No References)" {
    if (-not $script:testExporterId) {
        Write-Host "    Skipping - no exporter available" -ForegroundColor Yellow
        return $false
    }
    ...
    $result = Invoke-ApiCall -Method "DELETE" -Endpoint "/api/exporters/$($script:testExporterId)"
    if ($result.Success) {
        Write-Host "    Exporter deleted successfully after removing references" -ForegroundColor Gray
        return $true
    } else {
        Write-Host "    Failed to delete exporter after cleanup: $($result.Error)" -ForegroundColor Red
        return $false
    }
}
...
$performanceResult = Test-Scenario "Performance Test" {
    ...
    # Create an exporter for performance testing with unique name
    $perfTimestamp = Get-Date -Format "HHmmssffff"
    $perfExporterData = @{
        name = "Performance Test Exporter $perfTimestamp"
        version = "3.0"
        description = "Exporter for performance testing"
        protocolId = $script:testProtocolId
        inputSchema = "{}"
    }

    $perfExporterResult = Invoke-ApiCall -Method "POST" -Endpoint "/api/exporters" -Body $perfExporterData
    if (-not $perfExporterResult.Success) {
        Write-Host "    Failed to create performance test exporter" -ForegroundColor Red
        return $false
    }

    $perfExporterId = $perfExporterResult.Data.id

    # Measure validation time
    $startTime = Get-Date
    $deleteResult = Invoke-ApiCall -Method "DELETE" -Endpoint "/api/exporters/$perfExporterId"
    $endTime = Get-Date
    ...
    if (-not $deleteResult.Success) {
        Write-Host "    Performance test exporter deletion failed: $($deleteResult.Error)" -ForegroundColor Red
        return $false
    }
...
Write-Host ""
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host "EXPORTERENTITY REFERENTIAL INTEGRITY TEST RESULTS" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
...
if ($script:passedTests -eq $script:totalTests) {
    ...
    Write-Host "✅ ExporterEntity deletion without references (allowed)" -ForegroundColor Green
    Write-Host "✅ ExporterEntity deletion with Step references (blocked)" -ForegroundColor Green
    Write-Host "✅ ExporterEntity update with Step references (blocked)" -ForegroundColor Green
    Write-Host "✅ ExporterEntity deletion after removing all references (allowed)" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "🎉 EXPORTERENTITY REFERENTIAL INTEGRITY VALIDATION SUCCESSFUL!" -ForegroundColor Green
