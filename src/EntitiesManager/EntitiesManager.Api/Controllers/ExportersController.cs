...
public class ExportersController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<ExporterEntity>>> GetAll()
    {
        var userContext = User.Identity?.Name ?? "Anonymous";

        _logger.LogInformation("Starting GetAll exporters request. User: {User}, RequestId: {RequestId}",
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully retrieved all exporter entities. Count: {Count}, User: {User}, RequestId: {RequestId}",
        ...
        {
            _logger.LogError(ex, "Error retrieving all exporter entities. User: {User}, RequestId: {RequestId}",
                userContext, HttpContext.TraceIdentifier);
    ...
    public async Task<ActionResult<object>> GetPaged([FromQuery] int page = 1, [FromQuery] int pageSize = 10)
    {
        ...
        _logger.LogInformation("Starting GetPaged exporters request. Page: {Page}, PageSize: {PageSize}, User: {User}, RequestId: {RequestId}",
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully retrieved paged exporter entities. Page: {Page}, PageSize: {PageSize}, Count: {Count}, TotalCount: {TotalCount}, TotalPages: {TotalPages}, User: {User}, RequestId: {RequestId}",
        ...
        {
            _logger.LogError(ex, "Error retrieving paged exporter entities. Page: {Page}, PageSize: {PageSize}, User: {User}, RequestId: {RequestId}",
                page, pageSize, userContext, HttpContext.TraceIdentifier);
            return StatusCode(500, "An error occurred while retrieving exporter entities");
        }
    }
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<ExporterEntity>> GetById(Guid id)
    {
        var userContext = User.Identity?.Name ?? "Anonymous";

        _logger.LogInformation("Starting GetById exporter request. Id: {Id}, User: {User}, RequestId: {RequestId}",
        ...
        try
        {
            ...
            if (entity == null)
            {
                _logger.LogWarning("Exporter entity not found. Id: {Id}, User: {User}, RequestId: {RequestId}",
                    id, userContext, HttpContext.TraceIdentifier);
        ...
        {
            _logger.LogError(ex, "Error retrieving exporter entity by ID. Id: {Id}, User: {User}, RequestId: {RequestId}",
                id, userContext, HttpContext.TraceIdentifier);
    ...
    public async Task<ActionResult<IEnumerable<ExporterEntity>>> GetByName(string name)
    {
        ...
        _logger.LogInformation("Starting GetByName exporter request. Name: {Name}, User: {User}, RequestId: {RequestId}",
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully retrieved exporter entities by name. Name: {Name}, Count: {Count}, User: {User}, RequestId: {RequestId}",
        ...
        {
            _logger.LogError(ex, "Error retrieving exporter entities by name. Name: {Name}, User: {User}, RequestId: {RequestId}",
                name, userContext, HttpContext.TraceIdentifier);
            return StatusCode(500, "An error occurred while retrieving exporter entities");
        }
    }
    ...
    public async Task<ActionResult<IEnumerable<ExporterEntity>>> GetByVersion(string version)
    {
        var userContext = User.Identity?.Name ?? "Anonymous";

        _logger.LogInformation("Starting GetByVersion exporter request. Version: {Version}, User: {User}, RequestId: {RequestId}",
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully retrieved exporter entities by version. Version: {Version}, Count: {Count}, User: {User}, RequestId: {RequestId}",
        ...
        {
            _logger.LogError(ex, "Error retrieving exporter entities by version. Version: {Version}, User: {User}, RequestId: {RequestId}",
                version, userContext, HttpContext.TraceIdentifier);
            return StatusCode(500, "An error occurred while retrieving exporter entities");
        }
    }
    ...
    [HttpPost]
    public async Task<ActionResult<ExporterEntity>> Create([FromBody] ExporterEntity entity)
    {
        var userContext = User.Identity?.Name ?? "Anonymous";
        ...
        {
            _logger.LogWarning("Model validation failed for Create exporter request. ValidationErrors: {ValidationErrors}, User: {User}, RequestId: {RequestId}",
    ...
    [HttpPut("{id:guid}")]
    public async Task<ActionResult<ExporterEntity>> Update(Guid id, [FromBody] ExporterEntity entity)
    {
        var userContext = User.Identity?.Name ?? "Anonymous";
        var compositeKey = entity?.GetCompositeKey() ?? "Unknown";
        ...
        if (!ModelState.IsValid)
        {
            _logger.LogWarning("Model validation failed for Update exporter request. Id: {Id}, ValidationErrors: {ValidationErrors}, User: {User}, RequestId: {RequestId}",
        ...
        if (id != entity!.Id)
        {
            _logger.LogWarning("ID mismatch in Update exporter request. UrlId: {UrlId}, BodyId: {BodyId}, User: {User}, RequestId: {RequestId}",
        ...
        try
        {
            ...
            if (existing == null)
            {
                _logger.LogWarning("Exporter entity not found for update. Id: {Id}, User: {User}, RequestId: {RequestId}",
                    id, userContext, HttpContext.TraceIdentifier);
                return NotFound($"Exporter with ID {id} not found");
            }
        ...
        {
            _logger.LogWarning("Exporter entity not found during update operation. Id: {Id}, User: {User}, RequestId: {RequestId}",
                id, userContext, HttpContext.TraceIdentifier);
            return NotFound($"Exporter with ID {id} not found");
        }
        catch (Exception ex)
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        _logger.LogInformation("Starting Delete exporter request. Id: {Id}, User: {User}, RequestId: {RequestId}",
            id, userContext, HttpContext.TraceIdentifier);
        ...
        try
        {
            ...
            if (existing == null)
            {
                _logger.LogWarning("Exporter entity not found for deletion. Id: {Id}, User: {User}, RequestId: {RequestId}",
                    id, userContext, HttpContext.TraceIdentifier);
                return NotFound($"Exporter with ID {id} not found");
            }
            ...
            if (!deleted)
            {
                _logger.LogError("Failed to delete exporter entity. Id: {Id}, User: {User}, RequestId: {RequestId}",
                    id, userContext, HttpContext.TraceIdentifier);
        ...
        {
            _logger.LogError(ex, "Error deleting exporter entity. Id: {Id}, User: {User}, RequestId: {RequestId}",
                id, userContext, HttpContext.TraceIdentifier);
            return StatusCode(500, "An error occurred while deleting the exporter");
        }
    }
}
