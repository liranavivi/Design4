...
public class GetImporterQueryConsumer : IConsumer<GetImporterQuery>
{
    private readonly IImporterEntityRepository _repository;
    private readonly ILogger<GetImporterQueryConsumer> _logger;

    public GetImporterQueryConsumer(IImporterEntityRepository repository, ILogger<GetImporterQueryConsumer> logger)
    {
        _repository = repository;
        _logger = logger;
    }

    public async Task Consume(ConsumeContext<GetImporterQuery> context)
    {
        using var activity = Activity.Current?.Source.StartActivity("GetImporterQuery");
        ...
        try
        {
            if (context.Message.Id.HasValue)
            {
                ...
                if (entity != null)
                    ...
                    await context.RespondAsync(new { Error = "Importer not found", Type = "NotFound" });
            }
            ...
            _logger.LogInformation("Successfully processed GetImporterQuery");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing GetImporterQuery");
