...
public class GetSourceQueryConsumer : IConsumer<GetSourceQuery>
{
    private readonly ISourceEntityRepository _repository;
    private readonly ILogger<GetSourceQueryConsumer> _logger;

    public GetSourceQueryConsumer(ISourceEntityRepository repository, ILogger<GetSourceQueryConsumer> logger)
    {
        _repository = repository;
        _logger = logger;
    }

    public async Task Consume(ConsumeContext<GetSourceQuery> context)
    {
        using var activity = Activity.Current?.Source.StartActivity("GetSourceQuery");
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully processed GetSourceQuery");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing GetSourceQuery");
